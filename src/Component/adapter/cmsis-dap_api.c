...
static int luaApi_adapter_dap_multi_read(lua_State *L) {
  ...
  if (cmdapObj->DapMultiRead(cmdapObj, type, reg, count, buff) !=
      ADPT_SUCCESS) {
    return luaL_error(L, "Insert to instruction queue failed!");
  }
  // 执行队列
...
static int luaApi_adapter_dap_multi_write(lua_State *L) {
  ...
  if (cmdapObj->DapMultiWrite(cmdapObj, type, reg, transCnt >> 2, buff) !=
      ADPT_SUCCESS) {
    return luaL_error(L, "Insert to instruction queue failed!");
  }
  // 执行队列
...
static int luaApi_cmsis_dap_new(lua_State *L) {
  ...
  Adapter *cmdapObj =
      CAST(Adapter *, lua_newuserdata(L, sizeof(Adapter)));  // +1
  // 创建CMSIS-DAP对象
...
static int luaApi_cmsis_dap_connect(lua_State *L) {
  ...
  // 分配空间
  uint16_t *vid_pid_buff =
      lua_newuserdata(L, (len_VIDs_PIDs << 1) * sizeof(uint16_t));
...
static int luaApi_cmsis_dap_transfer_configure(lua_State *L) {
  ...
  if (CmdapTransferConfigure(cmdapObj, idleCycle, waitRetry, matchRetry) !=
      ADPT_SUCCESS) {
...
static int RegisterApi_CmsisDap(lua_State *L, void *opaque) {
  // 创建CMSIS-DAP类型对应的元表
  LuaApiNewTypeMetatable(L, CMDAP_LUA_OBJECT_TYPE, luaApi_cmsis_dap_gc,
                         lib_cmdap_oo);
