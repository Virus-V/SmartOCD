  ...
  if (cmdapObj->connected != TRUE) {
    for (; vids[idx] && pids[idx]; idx++) {
      log_debug("Try connecting vid: 0x%02x, pid: 0x%02x usb device.",
                vids[idx], pids[idx]);
      if (cmdapObj->usbObj->Open(cmdapObj->usbObj, vids[idx], pids[idx],
        ...
        // 选择配置和声明接口
        if (cmdapObj->usbObj->SetConfiguration(cmdapObj->usbObj, 0) !=
            USB_SUCCESS) {
          log_warn("USB.SetConfiguration failed.");
          return ADPT_ERR_TRANSPORT_ERROR;
        }
        if (cmdapObj->usbObj->ClaimInterface(cmdapObj->usbObj, 3, 0, 0, 3) !=
            USB_SUCCESS) {
          log_warn("USB.SetConfiguration failed.");
          return ADPT_ERR_TRANSPORT_ERROR;
        }
...
static int dapInit(struct cmsis_dap *cmdapObj) {
  ...
  if (cmdapObj->PacketSize > cmdapObj->usbObj->readMaxPackSize) {
    ...
    cmdapObj->usbObj->Read(
        cmdapObj->usbObj,
        cmdapObj->respBuffer + cmdapObj->usbObj->readMaxPackSize, rest_len, 0,
        &transferred);
  }
  ...
  switch (mode) {
    ...
    case 0:
      ...
      INTERFACE_CONST_INIT(enum transfertMode,
  ...
  switch (mode) {
    ...
    case ADPT_MODE_JTAG:
      ...
      if (*CAST(uint8_t *, cmdapObj->respBuffer + 1) != CMDAP_PORT_JTAG) {
        ...
        INTERFACE_CONST_INIT(enum JTAG_TAP_State, cmdapObj->adaperAPI.currState,
                             JTAG_TAP_RESET);
        return ADPT_SUCCESS;
      }
    default:
...
MAKE_PACKT:
  ...
  for (; seqIdx < sequenceCount; seqIdx++) {
    ...
    uint8_t tdiByte = (tckCount + 7) >>
                      3;  // 将TCK个数圆整到字节，表示后面跟几个byte的tdi数据
...
static int CmdapTransfer(Adapter self, uint8_t index, int sequenceCnt,
  ...
  *okSeqCnt = 0;
  ...
  int idx = 0, outIdx = 0, packetStartIdx,
      sendPackCnt =
          0;  // 指向下一个sequence控制字节的索引，数据包的开始索引，发送数据包的个数
  int transferred;
...
MAKE_PACKT:
  ...
  sendPackBuff[1] =
      index;  // DAP index, JTAG ScanChain 中的位置，在SWD模式下忽略该参数
  ...
  BOOL result = TRUE;
  for (int readPackCnt = 0; readPackCnt < sendPackCnt;
       readPackCnt++) {  // NOTIC 这个循环也就执行一次
    dapRead(cmdapObj, &transferred);
    log_trace("Read %d byte.", transferred);
...
static int CmdapTransferBlock(Adapter self, uint8_t index, int sequenceCnt,
  ...
  int sentPacketMaxCnt =
      (cmdapObj->PacketSize - 5) >> 2;  // 发送数据包可以装填的数据个数
  int readPacketMaxCnt =
      (cmdapObj->PacketSize - 4) >> 2;  // 接收数据包可以装填的数据个数
      ...
      while (restCnt > 0) {
        if (restCnt > sentPacketMaxCnt) {
          *CAST(uint16_t *, buff + 2) = sentPacketMaxCnt;
          buff[4] = seq;
          // 拷贝数据
          memcpy(buff + 5, data + readCnt,
                 sentPacketMaxCnt << 2);  // 将数据拷贝到
...
static int dapReset(Adapter self, enum targetResetType type) {
  ...
  switch (type) {
    ...
    case ADPT_RESET_DEBUG_RESET:
      ...
      } else if (cmdapObj->currTransMode ==
                 ADPT_MODE_SWD) {  // 发送SWD Line Reset
...
static int executeJtagCmd(Adapter self) {
  ...
  list_for_each_entry(cmd, &cmdapObj->JtagInsQueue, list_entry) {
    switch (cmd->type) {
      case JTAG_INS_STATUS_MOVE:  // 状态机切换
        if (cmd->instr.statusMove.toState ==
            tempState) {  // 如果要到达的状态与当前状态一致,则跳过该指令
          continue;
        }
        do {
          // 电平状态数乘以2，因为CMSIS-DAP的JTAG指令格式是一个seqinfo加一个tdi数据，而且TAP状态机的任意两个状态切换tms不会多于8个时钟
          TMS_SeqInfo tmsSeq = JtagGetTmsSequence(
              tempState, cmd->instr.statusMove.toState);  // 高8位为时序信息
      ...
      case JTAG_INS_EXCHANGE_DATA:  // 交换TDI和TDO之间的数据
        if (tempState != JTAG_TAP_DRSHIFT &&
            tempState != JTAG_TAP_IRSHIFT) {  //检查当前TAP状态
          log_error(
        ...
        do {
          ...
          readBuffLen +=
              readCnt + 1;  // 最后+1是跳出SHIFT-xR的状态需要的一位数据
        } while (0);
        // 更新当前JTAG状态机到下一个状态
        tempState++;
        break;
  ...
  list_for_each_entry_safe(cmd, cmd_t, &cmdapObj->JtagInsQueue, list_entry) {
    ...
    int restBit =
        (cmd->instr.exchangeData.bitCount - 1) & 0x7;  // 剩余二进制位个数
    ...
    switch (cmd->type) {
      case DAP_INS_RW_REG_SINGLE:
        *(writeBuff + writeCnt++) = cmd->instr.singleReg.request;
        ...
        //				log_debug("-----------------------------");
        //				log_debug("%s %s Reg %x.",
        //					cmd->instr.singleReg.request & 0x2
        //?
        //"Read" : "Write",
        //cmd->instr.singleReg.request & 0x1 ? "AP" : "DP",
        //					cmd->instr.singleReg.request &
        // 0xC); 				if((cmd->instr.singleReg.request & 0x2) ==
        // 0){ 					log_debug("0x%08X", cmd->instr.singleReg.data.write);
        //				}
        //			}while(0);
      ...
      case DAP_INS_RW_REG_MULTI:
        ...
        *(writeBuff + writeCnt++) = cmd->instr.multiReg.request;
        ...
        if ((cmd->instr.multiReg.request & 0x2) == 0) {
          // XXX 小端字节序
          memcpy(writeBuff + writeCnt,
                 CAST(uint8_t *, cmd->instr.multiReg.data),
